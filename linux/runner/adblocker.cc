...
void adblocker_init(BrowserApp *app) {
  ...
  // Load the filter immediately - with retry logic
  adblocker_enable(app, TRUE);
  
  // Schedule a delayed reload to ensure filter is loaded
  g_timeout_add(500, (GSourceFunc)adblocker_reload_filter, app);
}
...
gboolean adblocker_reload_filter(BrowserApp *app) {
  if (!app || !app->filter_store) return FALSE;
  
  g_print("AdBlocker: Reloading filter (retry)...\n");
  webkit_user_content_filter_store_load(app->filter_store, "default-ads", NULL,
                                        (GAsyncReadyCallback)on_filter_loaded, app);
